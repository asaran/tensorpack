...
class EmbeddingModel(ModelDesc):
    ...
    def embed(self, x, b, nfeatures=embed_dim):
        ...
        with slim.arg_scope([slim.layers.fully_connected], weights_regularizer=slim.l2_regularizer(1e-5)):
            ...
            # Extra layers
            net = slim.layers.fully_connected(net, 4096, scope ='fc8')
            net = slim.layers.dropout(net, 0.5, scope='dropout8')
            net = slim.layers.fully_connected(net, 4096, scope ='fc9')
            net = slim.layers.dropout(net, 0.5, scope='dropout9')

            #try:
            #    _ = embeddings.shape
            #    embeddings = np.array(embeddings, slim.layers.fully_connected(net, nfeatures, activation_fn=None, scope='fc8_'+str(i)))
            #except:
            embeddings = slim.layers.fully_connected(net, nfeatures, activation_fn=None, scope='fc10')
...
def get_config(model, algorithm_name):
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            ScheduledHyperParamSetter('learning_rate', [(100, 1e-4), (200, 1e-5), (300,1e-6)])
        ],
        extra_callbacks=[
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelname', help='directory name of the saved model', type=str)
