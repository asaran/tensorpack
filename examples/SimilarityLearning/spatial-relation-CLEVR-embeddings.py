    ...
    def embed(self, x, p, nfeatures=embed_dim):
        ...
        with slim.arg_scope([slim.layers.fully_connected], weights_regularizer=slim.l2_regularizer(1e-5)):
            ...
            if list_split > 0:
                ...
                pos_float = tf.cast(p, tf.float32)
                net = tf.concat(axis=2, values=[fc6_embed,pos_float])
                print('Printing multi pos shape..')
                print(net.get_shape())
                ...
                net = tf.reshape(net,[tf.shape(net)[1]*list_split,4102])
                print('Printing reshaped..')
                print(net.get_shape())
            else:
                pos_float = tf.cast(p, tf.float32)
                net = tf.concat(axis=1, values=[net,pos_float])
                print('Printing single pos shape..')
                print(net.get_shape())
...
class SiameseModel(EmbeddingModel):
    ...
    def _get_inputs(self):
        return [InputDesc(tf.float32, (None, 224, 224), 'input'),
                InputDesc(tf.int32, (None, 6), 'pos'),
                InputDesc(tf.float32, (None, 224, 224), 'input_y'),
                InputDesc(tf.int32, (None, 6), 'pos_y'),
                InputDesc(tf.int32, (None,), 'label')]

    def _build_graph(self, inputs):
        # get inputs
        img_x, pos_x, img_y, pos_y, label = inputs
        # embed them
        x_embed, y_embed = self.embed([img_x, img_y], [pos_x, pos_y])
...
class TripletModel(EmbeddingModel):
    @staticmethod
    def get_data():
        ds = DatasetTriplets('/media/internal/asaran/CLEVR/train_relations.json','train')
        ds = AugmentImageComponent(ds, [imgaug.Resize((224, 224))])
    ...
    def _get_inputs(self):
        return [InputDesc(tf.float32, (None, 224, 224, 3), 'input'),
                ...
                InputDesc(tf.int32, (None,6), 'pos_p'),
                InputDesc(tf.float32, (None, 224, 224, 3), 'input_n'),
                InputDesc(tf.int32, (None, 6), 'pos_n')
                ]
    ...
    def _build_graph(self, inputs):
        global embed_dim
        print(len(inputs))
        img_a, pos_a, img_p, pos_p, img_n, pos_n = inputs
        # scaling the bb coordinates wrt image
        pos_a = tf.scalar_mul(224,pos_a)
        pos_p = tf.scalar_mul(224,pos_p)
        pos_n = tf.scalar_mul(224,pos_n)
        a_embed, p_embed, n_embed = self.embed([img_a, img_p, img_n,], [pos_a, pos_p, pos_n], embed_dim)
...
class SoftTripletModel(TripletModel):
    def loss(self, a, p, n):
        print('Calling soft triplet loss..')
        l ,pd, nd = symbf.soft_triplet_loss(a, p, n, scope="loss")
        return l, pd, nd
...
def visualize(model_path, model, algo_name):
    ...
    relation_labels = {0:'left', 1:'right', 2:'front', 3:'behind'}
    circles = []
    classes = []
...
def evaluate_random(model_path, model, algo_name):
    ...
    # get train data
    dt = get_test_data('/media/internal/asaran/CLEVR/train_relations.json')
    dt.reset_state()
    ...
    for dp in ds.get_data():
        img, pos, label = dp
        embed_test_batch = pred([img, pos])[0]
        dist = {}
        for i in range(BATCH_SIZE):
