...
class Dataset(RNGDataFlow):
    ...
    def __init__(self, pathfile, train_or_test, shuffle=False):
        ...
        print('Total data points: '+str(len(idxs)))
        for i in range(len(idxs)/50):
            k = idxs[i]
            element = data[k]
            ...
            self.position.append(np.array(element['position']))
    ...
    def get_data(self):
        ...
        for k in idxs:
            yield [self.images[k], self.position[k], self.labels[k]]

if __name__ == '__main__':
    ds = Dataset('/media/internal/asaran/CLEVR/train_relations.json', 'train', shuffle=True)
    ds.reset_state()
