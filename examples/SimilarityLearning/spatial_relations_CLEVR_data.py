...
def get_digits_by_label(images, labels, position):
    img_data = []
    pos_data = []
    for clazz in range(0, 4):
        ...
        pos_clazz = [position[i] for i in clazz_filter]
        img_data.append(images_clazz)
        pos_data.append(pos_clazz)
...
class DatasetPairs(Dataset):
    ...
    def __init__(self, pathFile, train_or_test):
        ...
        self.img_dict, self.pos_dict = get_digits_by_label(self.images, self.labels, self.position)
        assert(len(self.img_dict)==len(self.pos_dict))
    ...
    def pick(self, label):
        ...
        return self.img_dict[label][idx].astype(np.float32), self.pos_dict[label][idx]

    def get_data(self):
        while True:
            y = self.rng.randint(2)
            if y == 0:
                pick_label, pick_other = self.rng.choice(4, size=2, replace=False)
            else:
                pick_label = self.rng.randint(4)
